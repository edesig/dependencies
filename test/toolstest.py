...
class TestDynamicDict(unittest.TestCase):
    ...
    def test_fibonacci(self):
        Fibonacci = dynamicdict(lambda k: Fibonacci[k - 1] + Fibonacci[k - 2],
                                {0: 0, 1: 1})
        self.assertEqual(218922995834555169026, Fibonacci[99], "Fibonacci miscalculated")
        return
...
class TestIdSet(unittest.TestCase):
    ...
    def test(self):
        ...
        for element in s:
            ...
            self.assertEqual(s.id(element), s[element], ".id is not equals to .__getitem__")
        s = idset()
        s.append(range(0, 10, 2))
        for element in s:
            self.assertEqual(element / 2, s.id(element), "Perhaps something with the default constructor")
        self.assertTrue(2 in s, "Operator in doesn't works well.")
        self.assertFalse(3 in s, "Operator in doesn't works well.")
        self.assertSetEqual(set(range(1, 10, 2)), set(range(10)) - s, "set difference is not correct")
